...
<template>
	<el-container>
		<template>
			<el-main>
				<div class="jiekuan"><i class="el-icon-s-home"></i>首页/贷中管理/合同管理</div>
					...
					<el-table class="ju" ref="multipleTable" :data="tableData" tooltip-effect="dark" style="width: 100%;">
						...
						<el-table-column prop="" label="操作" width="105">
							<template slot-scope="scope">
								<div>
									<div><router-link to="/index/DaiZhong/HeTongGuanLi2" tag="span">{{ scope.row.caozuo }}</router-link></div>
									<div @click="fangkuanshenpi">{{ scope.row.caozuo2 }}</div>
								</div>
							</template>
						</el-table-column>
					</el-table>
...
<script>
	export default{
		...
		methods: {
			...
			// 提交放款审批
			fangkuanshenpi() {
				this.$confirm('请确认借款合同准确无误？是否继续', '提示', {
